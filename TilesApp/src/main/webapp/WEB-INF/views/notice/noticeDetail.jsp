...
<table class="table" border="1" style="margin: auto; width: 80%">
	<thead>
		<tr style="border: 1px solid blue;">
			<th colspan="4" style="text-align: center">댓글정보</th>
		</tr>
	</thead>
	<tbody id="list">
	</tbody>
...
<script>
let nid = ${vo.noticeId};
let logid = '${id}';  // 따옴표가 있어야지 문자열로 본다.
	...
	function deleteReply(replyId){
		...
		// $("tr[data-id="+replyId+"]").remove();  // data-id값이 replyId를 가져오겠다.
	}
		console.log("nid: "+nid);
	$("#addReply").on('click',function(){   // .on은 메소드 등록.
		...
		$.ajax({
			url:'addReply.do',
			method:'post',
			data:{title:title, subject:subject, writer:writer, nid:nid}, // {parameter값:매개값}
			success: function(result){
				console.log(result);
	...
	function makeTr(reply){
		...
		let tr1 = $('<tr />').attr('data-id',reply.replyId).append(  // 태그를 만들겠다. attr = setAttribute
				  $("<td />").html("<b>번호:</b> "+reply.replyId),  // td태그를 만들고 innerText로 넣겠다.
				  ...
				  $("<td colspan='4'/>").html(
						 function(){
							if(logid == reply.replyWriter){
								return  reply.replySubject+"<button onclick='deleteReply("+reply.replyId+")' class='btn btn-warning' style='float:right;'>삭제</button>"
							}else{
		...
		console.log(tr1, tr2);
		$("#list").prepend(tr1, tr2); // id값이 list인 녀석을 찾아오겠다. 최신글이 위로 올라오도록.
	}
